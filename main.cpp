...
int waveForm = 5;                                 //  Forma de onda
int periodo = 120;                                 //  Período
int maxT = 90, minT = 50;                         //  Temperatura máxima e mínima
double aux = 0;
bool estado = true;
double Setpoint1 = 20, Input1 = 0, Output1 = 0;   //  Referência, entrada e saída para peltier fria
...
double Kp1 = 4, Ki1 = 3, Kd1 = 0;
double Kp2 = 4 , Ki2 = 3, Kd2 = 0;                    //  Parâmetros PID dos controladores
int report = 0;
...
PID PIDcold(&Input1, &Output1, &Setpoint1, Kp1, Ki1, Kd1, DIRECT);
PID PIDhot(&Input2, &Output2, &Setpoint2, Kp2, Ki2, Kd2, DIRECT);
...
void setup() {
  ...
  ads.setGain(GAIN_ONE);           // 1x gain   +/- 4.096V  1 bit =       2mV         0.125mV       Utilizado
...
void changeParameter(String rString)
{
  if(rString != 0)
  {
    ...
    else
    {
      ...
      str_aux = str_data[0];
      periodo = str_aux.toInt();
      str_aux = str_data[1];
      waveForm = str_aux.toInt();
      str_aux = str_data[2];
      upper = str_aux.toDouble();
      str_aux = str_data[3];
      lower = str_aux.toDouble();
    }
  }
}
